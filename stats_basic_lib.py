...
#卡方分布
random = np.random.chisquare(5,size=100)
stats.chi.pdf(5,random)
stats.chi.cdf(5,random)
#F分布
random = np.random.f(5,4,size=100)
...
#残差分析
plt.scatter(model.fittedvalues,model.resid) #残差图(0附近随机分布)
plt.xlabel(u'拟合值')
plt.ylabel(u'残差')
sm.qqplot(model.resid_pearson,stats.norm,line='45') #检验残差的正态性(直线附近)
plt.scatter(model.fittedvalues,model1.resid_pearson**0.5) #检验残差的同方差性(均匀带状)
plt.xlabel(u'拟合值')
plt.ylabel(u'标准化残差的平方根')
# ANOVA
table = anova.anova_lm(model) #p=PR 原假设:不同行业对return没有影响 p<a则拒绝原假设
print(table)
